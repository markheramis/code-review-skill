...
1. Keep findings independently readable.
2. Do not omit `Limitations` when the review is incomplete.
3. Do not claim tests passed unless they were actually run.
4. Do not claim production behavior unless confirmed by production code, config, logs, or documentation.
5. Use `Unknown` instead of guessing.
6. Prefer `Request changes` when there are confirmed `High` or `Critical` findings.
7. Prefer `Approve with follow-ups` when only non-blocking `Low` or `Medium` findings remain.
8. Include no more than one recommendation per finding unless alternatives are explicitly useful.
9. Every finding should have a validation path.
10. Record audit tools, checks, temporary validation artifacts, and cleanup status when they were used.
11. Remove example-only finding blocks unless replacing them with real, evidenced findings.
